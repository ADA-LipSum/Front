...
function SortHead({
  ...
  label: string;
  field: keyof AdminPost;
  sort: SortConfig;
  onSort: (f: keyof AdminPost) => void;
  right?: boolean;
}) {
...
export default function PostsPage() {
  ...
  const [bulkDeleteTarget, setBulkDeleteTarget] = useState<{ uuid: string; writer: string } | null>(
    null,
  );
  ...
  const filtered =
    boardFilter === 'all' ? rawPosts : rawPosts.filter((p) => p.boardType === boardFilter);
    ...
    <div className="space-y-4">
                    ...
                    <TableCell>
                      <Badge variant="outline" className="text-xs">
                        ...
                        {post.communityCategory
                          ? ` · ${CATEGORY_LABELS[post.communityCategory] ?? post.communityCategory}`
                          : ''}
                      </Badge>
                    </TableCell>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              <Select value={reseqTable} onValueChange={(value) => value && setReseqTable(value)}>
      ...
      <AlertDialog open={!!bulkDeleteTarget} onOpenChange={() => setBulkDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              '{bulkDeleteTarget?.writer}'의 게시글을 모두 삭제하시겠습니까?
            </AlertDialogTitle>
            <AlertDialogDescription>
              이 작업은 되돌릴 수 없습니다. 해당 사용자가 작성한 모든 게시글이 삭제됩니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={deleteCommentConfirm} onOpenChange={setDeleteCommentConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              해당 댓글의 좋아요 및 자식 댓글도 함께 삭제됩니다. 이 작업은 되돌릴 수 없습니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>취소</AlertDialogCancel>
      ...
      <AlertDialog open={reseqConfirm} onOpenChange={setReseqConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              AUTO_INCREMENT 값을 현재 데이터 기준으로 재정렬합니다. 서비스 중 실행 시 주의하세요.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>취소</AlertDialogCancel>
