...
const INITIAL_REPORTS: MockReport[] = [
  ...
  {
    reportId: 3,
    reporterNickname: '세종대왕',
    targetNickname: '허위정보유포자',
    reason: '허위 정보 유포',
    contentType: 'POST',
...
function SortHead({
  ...
  label: string;
  field: keyof MockReport;
  sort: SortConfig;
  onSort: (f: keyof MockReport) => void;
}) {
...
export default function ReportsPage() {
  ...
  const filtered = reports.filter((r) => {
    const matchSearch =
      !search ||
      r.reporterNickname.toLowerCase().includes(search.toLowerCase()) ||
      r.targetNickname.toLowerCase().includes(search.toLowerCase());
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Select value={typeFilter} onValueChange={(v) => setTypeFilter(v ?? 'all')}>
              <SelectTrigger className="w-28 h-8">
                <SelectValue placeholder="유형">
                  {{ all: '전체 유형', ...TYPE_LABELS }[typeFilter] ?? typeFilter}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
            ...
            <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v ?? 'all')}>
              <SelectTrigger className="w-32 h-8">
                <SelectValue>
                  {{ all: '전체 상태', ...STATUS_LABELS }[statusFilter] ?? statusFilter}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
            ...
            <TableBody>
              {sorted.map((report) => (
                <TableRow key={report.reportId}>
                  ...
                  <TableCell className="text-sm max-w-45 truncate">{report.reason}</TableCell>
                  <TableCell>
                    ...
                    {report.status === 'PENDING' && (
                      <div className="flex gap-1.5">
                        ...
                        <Button
                          ...
                          onClick={() => handleStatusChange(report.reportId, 'DISMISSED')}
                        >
                          기각
                        </Button>
                      </div>
